...
def get_aes_lib():
    """Get the compiled AES library (compile if needed)"""
    global _aes_lib, _so_file_path
    
    if _aes_lib is not None:
        return _aes_lib
    
    current_dir = os.path.dirname(os.path.abspath(__file__))
    so_file = os.path.join(current_dir, "aes_round.so")
    c_file = os.path.join(current_dir, "aes_round.c")
    ...
    lib.ref_aes_round_c.restype = None
    
    # Cache the library
    _aes_lib = lib
    
    # Register cleanup function to run at exit
    atexit.register(_cleanup_aes_lib)
    
    return lib

def ref_aes_round(input_data, key_data):
    ...
    # Call C function
    lib.ref_aes_round_c(input_arr, key_arr, output_arr)
